...
class CustomerPaymentController extends Controller
{
    public function create(DeliveryRequest $delivery)
    {
        ...
        if ($isPostpaid && !in_array($delivery->status, ['completed', 'delivered'])) {
            return redirect()->route('customer.delivery-requests.show', $delivery->id)
                ->with('error', 'This delivery is not yet completed. You can pay once it\'s delivered.');
        }
    ...
    public function store(Request $request, DeliveryRequest $delivery)
    {
        ...
        DB::transaction(function () use ($delivery, $validated, $request, $paymentType, $paymentSource) {
            ...
            if ($delivery->deliveryOrder && $delivery->deliveryOrder->driver) {
                // NotificationService::notifyCollectorOfOnlinePayment($delivery->deliveryOrder->driver, $delivery);
            }
        });

                ...
                <div class="text-sm">
                    ...
                    <span class="value">
                        @if($waybill->deliveryRequest->payment_type === 'prepaid')
                            {{ $waybill->deliveryRequest->isPaid() ? 'PAID' : 'PENDING' }}
                        @else
                            {{ $order && $order->status === 'completed' ? 'COLLECTED' : 'TO BE COLLECTED' }}
                        @endif
                    </span>
                </div>
                @if($waybill->deliveryRequest->payment_type === 'prepaid' && $waybill->deliveryRequest->isPaid())
    ...
    <div class="footer border-t pt-4">
        <div class="flex">
            <div>
                ...
                <span class="value">{{ $order->driver->name ?? 'N/A' }}</span>
                @if($order && $order->truck)
                    <span class="text-xs">(Truck: {{ $order->truck->license_plate ?? 'N/A' }})</span>
                @endif
            </div>
            <div>
        ...
        <div class="notice">
            This waybill updates progressively as the delivery is processed. Final fields such as ETA, Driver, and Receiver are recorded upon completion.
        </div>
    </div>
</body>
</html>
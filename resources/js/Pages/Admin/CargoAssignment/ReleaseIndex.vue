                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-gray-100">
                  <div class="flex flex-col">
                    ...
                    <div class="mt-1 flex flex-wrap items-center gap-2 text-xs text-gray-500 dark:text-gray-300">
                      ...
                      <span v-if="order.delivery_request?.status" class="flex items-center gap-1">
            ...
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              <tr v-for="order in completedOrders?.data || []" :key="order.id">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <div class="flex justify-end space-x-2">
                    <SecondaryButton
                      @click="goToView(order.delivery_request?.id)"
                      class="inline-flex items-center"
                      title="View"
                      style="padding-left: 0.75rem; padding-right: 0.75rem;"
                    >
...
function handlePageChange(page) {
  router.get(route('cargo-assignments.delivery-completion.ready-for-release', { 
    ...
    preserveState: true,
    preserveScroll: true,
    only: ['orders', 'completedOrders', 'filters']
  });
}
...
watch([search, paymentType], debounce(([searchVal, typeVal]) => {
  router.get(route('cargo-assignments.delivery-completion.ready-for-release'), { 
    search: searchVal,
...
function goToRelease(orderId) {
  router.visit(route('cargo-assignments.delivery-completion.release', orderId));
}

function goToView(deliveryOrderId) {
  if (deliveryOrderId) {
    router.visit(route('deliveries.show', deliveryOrderId));
  }
}
</script>
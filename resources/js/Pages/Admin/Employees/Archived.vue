  ...
  <EmployeeLayout>
    <template #header>
      <div class="flex justify-between items-center px-6">
        <h2 class="text-xl font-semibold leading-tight text-gray-800">Disabled Accounts</h2>
        <SecondaryButton @click="viewActive">View Active Accounts</SecondaryButton>
      </div>
    </template>
        ...
        <div class="p-5 bg-white border-b border-gray-200">
          ...
          >
            ...
            <template #employee_id="{ row }">
              {{ row.employee_profile?.employee_id || 'N/A' }}
            </template>
            <template #region="{ row }">
            ...
            <template #actions="{ row }">
              <div class="flex space-x-2">
                <SecondaryButton @click="restoreEmployee(row)">Restore</SecondaryButton>
                <DangerButton @click="confirmDeleteEmployee(row)">Delete Permanently</DangerButton>
              </div>
            </template>
          </DataTable>
...
const columns = [
  ...
  { 
    ...
    header: 'Role', 
    sortable: true, 
    formatter: v => v ? v.charAt(0).toUpperCase() + v.slice(1) : 'N/A' 
  },
  { 
    field: 'employee_profile', 
    header: 'Employee ID', 
    sortable: true,
    formatter: p => p?.employee_id || 'N/A'
  },
  { 
    field: 'employee_profile', 
    header: 'Region', 
    sortable: true, 
    formatter: p => p?.region?.name || 'Not assigned' 
  },
  { 
   field: 'employee_profile', 
  header: 'Disabled On',
  sortable: true,
  formatter: p => p?.archived_at ? new Date(p.archived_at).toLocaleDateString() : 'N/A'
  },
...
const handleDelete = async () => {
  ...
  try {
    await router.delete(route('admin.employees.destroy', employeeToDelete.value.id), { 
      preserveScroll: true 
    });
    closeDeleteModal();

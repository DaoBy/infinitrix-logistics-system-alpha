      ...
      <div class="flex justify-between items-center px-4 sm:px-6">
        <div class="min-w-0 flex-1">
          <h2 class="text-lg sm:text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200 truncate">
            Resubmit Payment Collection
          </h2>
          <p class="mt-1 text-sm text-gray-600 dark:text-gray-400 hidden sm:block">
            Update payment collection details after rejection
          </p>
        </div>
        <SecondaryButton 
          @click="router.visit(route('collector.payments.index'))"
            ...
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="space-y-2 text-sm">
                  <!-- Rejection Reason -->
                  <div v-if="existingPayment.rejection_reason" class="text-red-700">
                    <span class="font-medium">Reason:</span> {{ existingPayment.rejection_reason }}
                  </div>
                  ...
                  <div class="flex flex-wrap gap-4 text-red-600">
                    <div class="flex items-center gap-1">
                      ...
                      <span class="text-xs">By: {{ existingPayment.rejected_by?.name || 'Admin' }}</span>
                    </div>
                    <div class="flex items-center gap-1" v-if="existingPayment.rejected_at">
                      <svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                      </svg>
                      <span class="text-xs">On: {{ formatDateTime(existingPayment.rejected_at) }}</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div v-if="receiptPreview" class="bg-white dark:bg-gray-800 shadow-sm rounded-lg border border-gray-200 dark:border-gray-700 p-6">
            <div class="text-center">
              ...
              <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">
                Preview of the updated payment proof
              </p>
              ...
              <p class="text-xs text-gray-500 dark:text-gray-400 mt-3">
                This updated payment proof will be submitted for verification
              </p>
            </div>
          </div>

          <!-- CURRENT RECEIPT SECTION -->
          <div v-if="existingPayment.receipt_image_url" class="bg-white dark:bg-gray-800 shadow-sm rounded-lg border border-gray-200 dark:border-gray-700 p-6">
            <div class="text-center">
              <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4 flex items-center justify-center gap-2">
                <svg class="w-5 h-5 text-gray-600 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 002.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 00-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 00.75-.75 2.25 2.25 0 00-.1-.664m-5.8 0A2.251 2.251 0 0113.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25zM6.75 12h.008v.008H6.75V12zm0 3h.008v.008H6.75V15zm0 3h.008v.008H6.75V18z"/>
                </svg>
                Current Receipt (Rejected)
              </h3>
              <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">
                This is the receipt that was previously rejected
              </p>
              <div class="flex justify-center">
                <div class="relative">
                  <img 
                    :src="existingPayment.receipt_image_url" 
                    class="max-h-64 rounded-lg border-2 border-gray-300 dark:border-gray-600 shadow-md" 
                    alt="Current receipt"
                  />
                  <div class="absolute top-2 right-2">
            ...
            <div class="p-4">
              <form @submit.prevent="submit">
                <div class="space-y-4">
                  ...
                  <div>
                    <InputLabel value="New Payment Proof *" />
                    <input
                      type="file"
                    ...
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                      Upload a clear photo of the updated payment proof. Maximum file size: 2MB
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <TextArea
                      v-model="form.notes"
                      :rows="3"
                      placeholder="Add any notes about the updated collection circumstances..."
...
const amountReceived = ref(Number(props.existingPayment.amount) || Number(props.delivery.total_price) || 0);

const form = useForm({
  method: 'cash', // Fixed to cash only
  amount: Number(props.existingPayment.amount) || Number(props.delivery.total_price) || 0,
  receipt_image: null,
...
const showPaymentAlerts = computed(() => {
  return isOverdue.value || props.delivery.payment_status === 'uncollectible';
});
...
function submit() {
  ...
  form.put(route('collector.payments.update', {
